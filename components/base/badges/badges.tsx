...
const withPillTypes = {
    [badgeTypes.pillColor]: {
        common: "size-max flex items-center whitespace-nowrap rounded-full ring-2 ring-inset",
        styles: filledColors,
    },
    [badgeTypes.badgeColor]: {
        common: "size-max flex items-center whitespace-nowrap rounded-md ring-2 ring-inset",
        styles: filledColors,
    },
    [badgeTypes.badgeModern]: {
        common: "size-max flex items-center whitespace-nowrap rounded-md ring-2 ring-inset shadow-sm",
        styles: {
            gray: {
...
const withBadgeTypes = {
    ...
    [badgeTypes.badgeModern]: {
        common: "size-max flex items-center whitespace-nowrap rounded-md ring-2 ring-inset bg-primary text-secondary ring-primary shadow-sm",
...
export const BadgeWithFlag = <T extends BadgeTypes>(props: BadgeWithFlagProps<T>) => {
    ...
    return (
        <span className={cx(colors.common, sizes[type][size], colors.styles[color].root)}>
            <img src={`https://www.untitledui.com/images/flags/${flag}.svg`} className="size-4 max-w-none rounded-full ring-2 ring-primary" alt={`${flag} flag`} />
            {children}
        </span>
    );
};
...
export const BadgeWithImage = <T extends BadgeTypes>(props: BadgeWithImageProps<T>) => {
    ...
    return (
        <span className={cx(colors.common, sizes[type][size], colors.styles[color].root)}>
            <img src={imgSrc} className="size-4 max-w-none rounded-full ring-2 ring-primary object-cover" alt="Badge image" />
            {children}
        </span>
    );
};
    ...
    return (
        <span className={cx(colors.common, sizes[type][size], colors.styles[color].root)}>
            ...
            <button
                ...
                className={cx(
                    "flex cursor-pointer items-center justify-center p-0.5 transition duration-100 ease-linear focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary",
